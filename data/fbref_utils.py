...
def get_player_id(driver, player_name: str, player_team: str) -> Optional[str]:
    ...
    if not country_code:
        print(f"Country not recognized per {player_name}")
        return None
    ...
    for item in search_items:
        try:
            ...
            try:
                alt_name = item.find_element(By.CSS_SELECTOR, "div.search-item-alt-names").text
            ...
            if all(token in found_name for token in name_tokens) or all(token in found_alt_name for token in name_tokens):
                if re.search(r'\b' + re.escape(country_code) + r'\b', block_text):
                    print(f"Match found: {link.text} | {href}")
                    parts = href.split("/")
                    if len(parts) > 5 and parts[5]:
                        return parts[5]
...
def extract_player_metastats(html: str) -> pl.DataFrame:
...
def compute_shooting_stats_average(season_data: int, num_years_back: int=3, min_minutes_90s: float=5.0):
    ...
    indices_2021 = [i for i, (season, _) in enumerate(season_data) if season in '2021-2022']
    if indices_2021:
    ...
    selected_seasons = season_data[max(0, reference_index - num_years_back):reference_index + 1]
    stats_sum = {}
    stats_count = {}
